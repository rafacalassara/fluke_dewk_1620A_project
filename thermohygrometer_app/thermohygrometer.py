...
class Thermohygrometer:
    ...
    def connect(self):
        try:
            self.instrument = self.rm.open_resource(f'TCPIP0::{self.ip}::{self.port}::SOCKET')
            ...
            print(f"Error connecting to DewK 1620A at {self.ip}: {e}")
            self.instrument = None
            return False
    ...
    def read_real_time_data(self, channel: str, delay):
        try:
            response = self.instrument.query(f'READ? {channel}')
            time.sleep(delay)
            parsed_response = self._parse_live_data_one_channel(response)
